      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="flex items-center space-x-1 sm:space-x-2">
            ...
            {/* Profile Button — shows Google photo if available */}
            <Button
              variant="ghost"
              size="icon"
            ...
            >
              <Avatar className="h-8 w-8">
                {userPhoto ? (
                  <AvatarImage src={userPhoto} alt={currentUser?.displayName ?? "User"} referrerPolicy="no-referrer" />
                ) : null}
                <AvatarFallback className="bg-gradient-to-br from-green-400 to-green-600 text-white text-sm">
                  {userPhoto ? userInitial : <User className="h-4 w-4" />}
                </AvatarFallback>
              </Avatar>
            </Button>
