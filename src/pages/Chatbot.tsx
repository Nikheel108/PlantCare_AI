...
export default function Chatbot() {
  ...
  useEffect(() => {
    const loadChatHistory = async () => {
      ...
      try {
        const response = await fetch(`${API_URL}/api/chat/${currentUser.uid}`);
  ...
  const handleSendMessage = async () => {
    ...
    if (currentUser) {
      fetch(`${API_URL}/api/chat`, {
        ...
        body: JSON.stringify({ userId: currentUser.uid, message: userMessage }),
    ...
    try {
      ...
      if (currentUser) {
        fetch(`${API_URL}/api/chat`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: currentUser.uid, message: botResponse }),
